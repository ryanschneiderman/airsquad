class Stats::Lineups::LineupStatRankingService
	...
	def call
		...
		stats = LineupStat.joins(:stat_list, :lineup).select(" stat_lists.stat as stat, stat_lists.is_percent as is_percent, stat_lists.advanced as advanced, lineups.team_id as team_id, lineup_stats.*, lineups.id as lineup_id, lineups.season_minutes as season_minutes").where('lineups.team_id' => @team_id, 'stat_lists.rankable'=> true, 'lineup_stats.is_opponent' => false, 'lineup_stats.season_id' => @season_id).sort_by{|e| [e.stat_list_id]}
		adv_stats = LineupAdvStat.joins(:stat_list, :lineup).select("stat_lists.stat as stat, stat_lists.advanced as advanced, lineups.team_id as team_id, lineup_adv_stats.*, lineups.id as lineup_id").where('lineups.team_id' => @team_id, 'lineup_adv_stats.is_opponent' => false, 'lineup_adv_stats.season_id' => @season_id).sort_by{|e| [e.stat_list_id]}
